...
class LuaTypes(object):
        ...
        # если тип value - с плавающей точкой
            ...
            if cls.is_float(target_type):
                if value.type == cls.float:
                    return builder.fpext(value, cls.float)
                else:
    ...
    def get_const_from_str(cls, luatype, const_value, ctx):
        # если const_value это строка
        if type(const_value) is str:
            # если тип luatype - символ
            if luatype == cls.char:
            ...
            # если тип luatype - с плавающей точкой
            elif luatype in [cls.float]:
                return luatype(float(const_value))

            # если тип luatype - целочисленный
            elif luatype in [cls.int]:
                return luatype(int(const_value))

            # если тип luatype - массив и его эл-ты - символы
            elif isinstance(luatype, ir.ArrayType) and luatype.element == cls.char:
                ...
                # создаём новую константу типа luatype с помощью ir.Constant,
                # передаём в нее массив байтов, представляющих строку str_val с кодировкой 'ascii'
                return ir.Constant(luatype, bytearray(str_val, 'ascii'))

            else:
                raise SemanticError(msg=f"No known conversion: {const_value} to {luatype}")
        else:
